/**
 * AutoDezire Client Suitability Engine
 * Client-side mirror of the backend suitability evaluation for instant reactivity.
 */
...
export function evaluateSuitability(vehicle, profile = {}) {
  ...
  const scores = { ...base };
  const userPriorities = profile.topPriorities || ['Safety', 'Ground Clearance', 'Comfort'];
  ...
  if (roadCondition.includes('Bad') || roadCondition.includes('Potholes') || (profile.ruralPercent || 10) > 25) {
  ...
  // Space & Practicality adjustments
  ...
  if (profile.luggageRequirement === 'Heavy' && (vehicle.bootSpace || 300) > 450) {
    scores.spacePracticality = Math.min(10, scores.spacePracticality + 1);
  } else if (profile.luggageRequirement === 'Heavy' && (vehicle.bootSpace || 300) < 200) {
  ...
  // Normalize scores to integer [1, 10]
  ...
  if (budgetStatus === 'Slightly Above Budget') {
    considerations.unshift(`Base price is slightly above your initial ₹${profile.budget || 14}L budget (within 15% margin)`);
  }
